...
# -----------------------------
# Plotly Visualization functions (IMPROVED & FANCY)
# -----------------------------
def plotly_match_outcome_probabilities(p_home, p_draw, p_away, home_team, away_team):
    """Plot outcome probabilities with Plotly."""
    ...
    fig.update_layout(
        title='Wahrscheinlichkeit des Spielausgangs',
...
def plotly_score_probabilities(score_probs):
    """Plot top score probabilities with Plotly."""
    ...
    fig.update_layout(
        title='Wahrscheinlichste Endergebnisse',
...
def plotly_total_goals_probabilities(total_goals_probs):
    """Visualize Over/Under probabilities with Plotly."""
    ...
    fig.add_trace(go.Bar(
        x=[f"Über {t}" for t in thresholds],
        y=over_probs,
        name='Über',
        marker_color='#2ECC71'
    ))
    ...
    fig.add_trace(go.Bar(
        x=[f"Unter {t}" for t in thresholds],
        y=under_probs,
        name='Unter',
        marker_color='#E74C3C'
    ))
    ...
    fig.update_layout(
        title='Über/Unter-Tore Wahrscheinlichkeit',
...
def plotly_xg_comparison(xg_home, xg_away, home_team, away_team):
    """Plot xG comparison with Plotly."""
    ...
    fig = go.Figure(go.Bar(
        x=categories,
        y=values,
        marker_color=['#4A90E2', '#E74C3C', '#4A90E2', '#E74C3C'],
        text=[f'{v:.2f}' for v in values],
        textposition='auto',
    ))
    
    fig.update_layout(
        title='Vergleich der erwarteten Tore (xG)',
        xaxis_title='Statistik',
        yaxis_title='Erwartete Tore pro Spiel',
        showlegend=False,
        height=400,
        template='plotly_white'
    )
...
with col1:
    ...
    # Fix for rank display
    try:
        home_rank = table_df[table_df['team'].str.strip().str.lower() == home_team.strip().lower()]
        if not home_rank.empty and 'rank' in home_rank.columns:
            home_rank_value = home_rank['rank'].iloc[0]
            home_rank_display = int(home_rank_value) if pd.notna(home_rank_value) else "N/A"
        else:
            home_rank_display = "N/A"
    except:
        home_rank_display = "N/A"
...
with col2:
    ...
    # Fix for rank display
    try:
        away_rank = table_df[table_df['team'].str.strip().str.lower() == away_team.strip().lower()]
        if not away_rank.empty and 'rank' in away_rank.columns:
            away_rank_value = away_rank['rank'].iloc[0]
            away_rank_display = int(away_rank_value) if pd.notna(away_rank_value) else "N/A"
        else:
            away_rank_display = "N/A"
    except:
        away_rank_display = "N/A"
...
# Visuals first
st.plotly_chart(plotly_match_outcome_probabilities(prediction['p_home'], prediction['p_draw'], prediction['p_away'], home_team, away_team), use_container_width=True)

tab1, tab2, tab3 = st.tabs(["Wahrscheinlichste Ergebnisse", "Über/Unter Tore", "Erwartete Tore (xG)"])
with tab1:
    st.plotly_chart(plotly_score_probabilities(prediction['score_probs']), use_container_width=True)
with tab2:
    st.plotly_chart(plotly_total_goals_probabilities(prediction['total_goals_probs']), use_container_width=True)
with tab3:
    xg_home = estimate_expected_goals(matches_df, home_team)
    xg_away = estimate_expected_goals(matches_df, away_team)
    st.plotly_chart(plotly_xg_comparison(xg_home, xg_away, home_team, away_team), use_container_width=True)
